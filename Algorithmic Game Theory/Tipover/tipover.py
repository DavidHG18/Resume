...
def find_solution(position, board):
    ...
    for move in moves(position, board):
        ...
        if solution is not None:
            ...
            x = next_move[0][0] # find new index after updating the board
            y = next_move[0][1]
            undo((x, y, move[2], move[3]), board)
    return None

# Function to undo a move on the board
def undo(command, board):
    x = command[0]
    y = command[1]
    ...
    if direction == (0, 1):
        # right direction, reverse by going left
        while board[x][y] != '.':
            board[x][y] = '.'
            counter += 1
            y -= 1
...
def tip_right(crate_height, position, board):
    ...
    for i in range(1, crate_height + 1):
        if board[position[0]][position[1] + i] != '.': # Condition where not empty
            return None
    return True
...
def tip_up(crate_height, position, board):
    ...
    if position[0] - crate_height < 0: # Out of bounds condition
        return None
    for i in range(1, crate_height + 1):
        if board[position[0] - i][position[1]] != '.': # Condition where not empty
            return None
    return True
